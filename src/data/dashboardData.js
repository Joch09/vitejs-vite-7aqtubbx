...
export function getBulletValues({
  ...
  return indicatorsByIndex.map((indicator) => {
    ...
    return {
      ...
      value:
        (isAccumulated ? record.va : record.vd)?.[dateIdx] ?? null,

      numerator:
        (isAccumulated ? record.na : record.nd)?.[dateIdx] ?? 0,

      denominator:
        (isAccumulated ? record.da : record.dd)?.[dateIdx] ?? 0,
    };
  });
}
